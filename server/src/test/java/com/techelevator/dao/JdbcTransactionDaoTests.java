...
public class JdbcTransactionDaoTests extends BaseDaoTests{
    ...
    @Test
    public void createNewTransactionWithValidAccountId(){
    ...
    @Test
    public void createNewTransactionWithoutValidAccountId(){
        Transaction transaction =new Transaction();
        transaction.setFromAccount(2008);
        transaction.setToUserAccount(1010);
        transaction.setTransactionAmount(BigDecimal.valueOf(1500));
        //transaction.setStatus("APPROVED");

        List<Transaction> list =sut.getTransactionsbyAccountId(transaction.getFromAccount());
        Assert.assertEquals("Account Id is not valid", 0, list.size());
